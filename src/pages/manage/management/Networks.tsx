...
import { getBranchNetworks, addBranchNetwork, updateBranchNetwork, deleteBranchNetwork } from 'shared/api/branchNetwork.provider';
...
export class Networks extends Component<NetworksProps> {
    ...
    // get all the lines in table 'branch_network' from the node server
    reloadAllBranchNetworks = async (): Promise<void> => {
        try {
    ...
    // function for adding a branch_network
    addBranchNetwork = async (Id: number, Name: string): Promise<void> => {
        await addBranchNetwork(Id, Name);
    }

    // function for updating a branch_network
    updateBranchNetwork = async (Id: number, Name: string): Promise<void> => {
        await updateBranchNetwork(Id, Name);
    }

    // function for deleting a branch_network
    deleteBranchNetwork = async (Id: number): Promise<void> => {
        await deleteBranchNetwork(Id);
    }

    init = async () => {
        await this.reloadAllBranchNetworks();
    }
    ...
    onRowInserting = (e: any) => {
        if (typeof this.state.networks.filter(network => network.Id === parseInt(e.data.Id, 10))[0] !== 'undefined') {
    ...
    onRowInserted(e: any) {
        this.addBranchNetwork(e.Id, e.Name).then(() => {
    ...
    onRowUpdated(e: any) {
        this.updateBranchNetwork(e.Id, e.Name).then(() => {
    ...
    onRowRemoved(e: any) {
        this.deleteBranchNetwork(e.Id).then(() => {
                ...
                <div className='grid-body'>
                    <div className='container'>
                        <DataGrid id={'gridContainer'}
                            ...
                            dataSource={new DataSource({ store: this.state.networks, paginate: false })}
                        ...
                        >
                            ...
                            <Export enabled={true} fileName={'networks'} />
                            <Editing
                                // mode={'row'}
                                mode={'popup'}
                            ...
                            <Column dataField={'Id'} caption={'קוד רשת'}><RequiredRule /></Column>
                            <Column dataField={'Name'} caption={'שם רשת'}><RequiredRule /></Column>
                        </DataGrid>
