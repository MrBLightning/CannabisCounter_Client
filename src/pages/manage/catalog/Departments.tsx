...
export class Departments extends Component<DepartmentsProps> {
    ...
    // function for reloading all Departments
    reloadAllDepartments = async (): Promise<void> => {
    ...
    addDepartment = async (Id: number, Name: string): Promise<void> => {
        await addDepartment(Id, Name);
    }

    // function for updating a Department
    updateDepartment = async (Id: number, Name: string): Promise<void> => {
    ...
    onRowInserting = (e: any) => {
        if (typeof this.state.departments.filter(department => department.Id === parseInt(e.data.Id, 10))[0] !== 'undefined') {
    ...
    onRowInserted(e: any) {
        ...
        this.addDepartment(e.Id, e.Name).then(() => {
    ...
    onRowUpdated(e: any) {
        ...
        this.updateDepartment(e.Id, e.Name).then(() => {
    ...
    onRowRemoved(e: any) {
        this.deleteDepartment(e.Id).then(() => {
            ...
            <div className='grid-wrapper'>
                {/* <div className='grid-header'>
                    <div className='container'>
                        <div className='header-text'>{"מחלקות"}</div>
                    </div>
                </div> */}
                <div className='grid-body'>
                    <div className='container'>
                        <DataGrid id={'gridContainer'}
                            ...
                            dataSource={new DataSource({ store: this.state.departments, paginate: false })}
                        ...
                        >
                            ...
                            <Editing
                                ...
                                />
                                <Popup title={'מחלקות'} showTitle={true} width={400} height={250}>
                                </Popup>
                                <Form>
                                    <Item itemType={'group'} >
                            ...
                            <Column dataField={'Id'} caption={'מספר מחלקה'}><RequiredRule /></Column>
                            <Column dataField={'Name'} caption={'מחלקה'}><RequiredRule /></Column>
                        </DataGrid>
