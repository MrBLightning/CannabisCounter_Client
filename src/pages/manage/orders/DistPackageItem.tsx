...
import { CatalogItem, Supsiryun, Group, Sapak, Siryun, SingleSupplierItem } from 'shared/interfaces/models/SystemModels';
import { getItems, getGroups, getSupSiryuns, getSapakim, addSiryun, updateSiryun, getSiryunTable } from 'shared/api/reserveFish.provider';
...
import { getSingleSupplierItems } from 'shared/api/distSingleItem.provider';

type ReserveFishsProps = {
    permission: RbacPermission,
    user: AuthUser;
} & RouteComponentProps;

type ReserveFishsState = {
...
export class ReserveFishs extends Component<ReserveFishsProps> {
    dataGrid: any | null = null;
    state: ReserveFishsState = {
        catalogs: [],
        supSiryuns: [],
        singleSupplierItems:[],
        siryuns: [],
    ...
    loadAllItems = async (): Promise<void> => {
        try {
            ...
            // Since we only work with PACKAGES in this screen we only need items
            // where ClassesId == 80 (Department חומרי אריזה)
            catalogs = catalogs.filter(catalog => catalog.ClassesId == 80);
            // we also need to exclude all items that have a single supplier from table single_supplier_item
    ...
    loadAllSupSiryuns = async (): Promise<void> => {
        try {
            ...
            // where ClassId == 6 (Department דגים)
            let availGroups = this.state.groups.filter(group => group.ClassId == 6);
