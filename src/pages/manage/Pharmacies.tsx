...
import { Pharmacy } from 'shared/interfaces/models/SystemModels';
import { getPharmacys, getLocations, addPharmacy, updatePharmacy, deletePharmacy } from 'shared/api/pharmacy.provider';
...
export class Pharmacies extends Component<PharmaciesProps> {
    ...
    addPharmacy = async (LocationId: number, Name: string, Address: string, Phone: string): Promise<void> => {
        await addPharmacy(LocationId, Name, Address, Phone);
    }

    updatePharmacy = async (Id: number, LocationId: number, Name: string, Address: string, Phone: string): Promise<void> => {
        await updatePharmacy(Id, LocationId, Name, Address, Phone);
    }
    ...
    onRowInserting = (e: any) => {
        if (typeof this.state.pharmacies.filter(pharmacy => pharmacy.Name === e.data.Name)[0] !== 'undefined') {
    ...
    onRowInserted(e: any) {
        ...
        this.addPharmacy(e.LocationId, e.Name, e.Address, e.Phone).then(() => {
    ...
    onRowUpdated(e: any) {
        ...
        this.updatePharmacy(e.Id, e.LocationId, e.Name, e.Address, e.Phone).then(() => {
            ...
            <div className='grid-wrapper'>
                <div className='grid-header'>
                    <div className='container'>
                        <div className='header-text'>{"בתי מרקחת"}</div>
                    </div>
                </div> 
                <div className='grid-body'>
                    <div className='container'>
                        <DataGrid id={'gridContainer'}
                            ...
                            dataSource={new DataSource({ store: this.state.pharmacies, paginate: false })}
                        ...
                        >
                            ...
                            <Export enabled={true} fileName={'pharmacies'} />
                            <Editing
                                // mode={'row'}
                                mode={'popup'}
                                ...
                                />
                                <Popup title={'בתי מרקחת'} showTitle={true} width={350} height={300}>
                                </Popup>
                                <Form>
                                    <Item itemType={'group'} >
                                        <Item dataField={'Name'} cssClass={'Form-item-texteditor'} />
                                        <Item dataField={'LocationId'} cssClass={'Form-item-texteditor'} />
                                        <Item dataField={'Address'} cssClass={'Form-item-texteditor'} />
                                        <Item dataField={'Phone'} cssClass={'Form-item-texteditor'} />
                                    </Item>
                                </Form>
                            </Editing>

                            <Column dataField={'Name'} caption={'שם'}><RequiredRule /></Column>
                            <Column dataField={'LocationId'} caption={'מיקום'} lookup={{
                                dataSource: () => this.state.locations,
                                displayExpr: "Name",
                                valueExpr: "Id"
                            }}><RequiredRule /></Column>
                            <Column dataField={'Address'} caption={'כתובת'}><RequiredRule /></Column>
                            <Column dataField={'Phone'} caption={'טלפון'}><RequiredRule /></Column>
                        </DataGrid>
                        {/*
