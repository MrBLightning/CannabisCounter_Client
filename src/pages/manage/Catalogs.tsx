...
import { getCannabisItems, addCannabisItem, updateCannabisItem, deleteCannabisItem } from 'shared/api/cannabisItem.provider';
import { getCategorys, addCategory, updateCategory, deleteCategory } from 'shared/api/category.provider';
...
export class Catalogs extends Component<CatalogsProps> {
    ...
    addItem = async (Id: number, Name: string): Promise<void> => {
        await addCategory(Id, Name);
    }

    updateItem = async (Id: number, Name: string): Promise<void> => {
    ...
    onRowInserting = (e: any) => {
        if (typeof this.state.categories.filter(category => category.Id === parseInt(e.data.Id, 10))[0] !== 'undefined') {
            uiNotify('אי אפשר להוסיף את הקוד הזה - הוא כבר קיים בטבלה', 'error');
            // cancel the event
            e.cancel = true;
        }
        if (typeof this.state.categories.filter(category => category.Name === e.data.Name)[0] !== 'undefined') {
            uiNotify('אי אפשר להוסיף את הקטגוריה הזו - הוא כבר קיים בטבלה', 'error');
            // cancel the event
            e.cancel = true;
        }
    }
    ...
    onRowUpdated(e: any) {
        ...
        this.updateItem(e.Id, e.Name).then(() => {
            ...
            <div className='grid-wrapper'>
                <div className='grid-header'>
                    <div className='container'>
                        <div className='header-text'>{"מוצרים"}</div>
                    </div>
                </div> 
                <div className='grid-body'>
                    <div className='container'>
                        <DataGrid id={'gridContainer'}
                            ...
                            dataSource={new DataSource({ store: this.state.categories, paginate: false })}
                        ...
                        >
                            ...
                            <Export enabled={true} fileName={'catalog'} />
                            <Editing
                                // mode={'row'}
                                mode={'popup'}
                                ...
                                />
                                <Popup title={'מוצרים'} showTitle={true} width={350} height={300}>
                                </Popup>
                                <Form>
                                    <Item itemType={'group'} >
                            ...
                            <Column dataField={'Id'} caption={'קוד'}><RequiredRule /></Column>
