...
import { getSuppliers, addSupplier, updateSupplier, deleteSupplier } from '../../shared/api/supplier.provider';
...
export class Suppliers extends Component<SuppliersProps> {
    ...
    addSupplier = async (Id: number, Name: string): Promise<void> => {
        await addSupplier(Id, Name);
    }

    updateSupplier = async (Id: number, Name: string): Promise<void> => {
    ...
    onRowInserting = (e: any) => {
        if (typeof this.state.suppliers.filter(supplier => supplier.Name === e.data.Name)[0] !== 'undefined') {
            uiNotify('אי אפשר להוסיף את הספק הזה - הוא כבר קיים בטבלה', 'error');
            // cancel the event
            e.cancel = true;
        }
        if (typeof this.state.suppliers.filter(supplier => supplier.Id === parseInt(e.data.Id, 10))[0] !== 'undefined') {
    ...
    onRowInserted(e: any) {
        ...
        this.addSupplier(e.Id, e.Name).then(() => {
    ...
    onRowUpdated(e: any) {
        ...
        this.updateSupplier(e.Id, e.Name).then(() => {
            ...
            <div className='grid-wrapper'>
                <div className='grid-header'>
                    <div className='container'>
                        <div className='header-text'>{"יצרנים"}</div>
                    </div>
                </div> 
                <div className='grid-body'>
                    <div className='container'>
                        <DataGrid id={'gridContainer'}
                            ...
                            dataSource={new DataSource({ store: this.state.suppliers, paginate: false })}
