...
export async function getSiryunByDate(CreateDate: string): Promise<Siryun[]> {
    return await getRequest<Siryun[]>('/order/reserveFish/' + CreateDate);
}

export async function getSiryunTable(CreateDate: number): Promise<any[]> {
    return await getRequest<any[]>('/order/reserveFish/buildTable/' + CreateDate);
}
...
export async function addSiryun(
    ...
    await postRequest('/order/reserveFish', { record });
}
...
export async function updateSiryun(
    ...
    await putRequest('/order/reserveFish', { recordWhere, record});
}

export async function deleteSiryun(Id: number) {
    await deleteRequest('/order/reserveFish/' + Id);
}
